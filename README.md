...
The current care upgrade adds secure chat/access, static HTML repair and approved **text-source reviews across 24 AI roles**. Source review provides cited advice and does not execute code or apply general repairs. See [source-review usage and rollout](docs/CARE-SOURCE-REVIEWS.md), the [current implementation record](docs/CARE-IMPLEMENTATION.md), and the [100-item requirement ledger](docs/upgrade-ledger.json). Fourteen bounded review tools are implemented; 52 proposed tools remain disabled. Run `npm run care:preflight -- TENANT_UUID` with deployment configuration loaded to inspect setup without invoking a model.
