...
test('affiliate can navigate its attributed financial workspace', async ({ page }) => {
  ...
  for (const section of ['Overview','Referral Link','Referrals','Attribution','Commissions','Transactions','Payouts','Notifications','Profile']) {
    await page.getByRole('navigation', { name: 'Affiliate navigation' }).getByRole('link', { name: section, exact: true }).click();
    const heading = section === 'Overview' ? /^Every connection,/ : section;
