...
export function ApplicationShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-shell${marketing ? ' app-shell--marketing' : ''}${careWorkspace ? ' app-shell--care' : ''}`}>
    ...
    <header className="app-header">
      ...
      {marketing ? <><nav aria-label="Primary navigation"><a href="#services">What we do</a><a href="#technology">All technologies</a><a href="#ai">AI capabilities</a><a href="#how-it-works">How it works</a><a href="#faq">FAQ</a></nav><div className="header-actions"><Link className="header-login" href="/sign-in">Sign in</Link><Link className="header-cta" href="/customer/register">Get started <span aria-hidden="true">→</span></Link></div><details className="zr-mobile-nav"><summary aria-label="Open navigation"><span aria-hidden="true">☰</span></summary><div onClick={(event) => { if ((event.target as HTMLElement).closest("a")) event.currentTarget.closest("details")?.removeAttribute("open"); }}><a href="#services">What we do</a><a href="#technology">All technologies</a><a href="#ai">AI capabilities</a><a href="#how-it-works">How it works</a><a href="#faq">FAQ</a><Link href="/sign-in">Sign in</Link></div></details></> : publicAuth ? <nav aria-label="Public navigation"><Link href="/">Home</Link></nav> : <nav aria-label="Primary navigation"><Link href="/">Home</Link><Link href="/dashboard">Dashboard</Link><Link href="/account">Sessions</Link><a href="http://localhost:4000/docs">API documentation</a></nav>}
    </header>
