...
export function HomeTechnologies({ roles, implementedTools, plannedTools }: { roles: Array<{ id: string; name: string }>; implementedTools: number; plannedTools: number }) {
  ...
  return <div className="zr-tech-directory">
    <p className="zr-tech-availability"><strong>{roles.length} source-review roles · {implementedTools} bounded tool implementations</strong><span>{plannedTools} planned tools remain unavailable. Includes saved-evidence tools and approval workflows. AI review needs a configured provider; your workspace shows enabled features.</span></p>
