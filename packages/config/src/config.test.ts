...
describe('environment configuration', () => {
  ...
  it('requires encrypted production transports and non-placeholder secrets', () => {
    const production = { ...valid, NODE_ENV: 'production', DATABASE_URL: 'postgresql://user:pass@db.example.com/app?sslmode=require', REDIS_URL: 'rediss://redis.example.com:6380', SESSION_SECRET: 'production-session-secret-with-high-entropy-value', MFA_ENCRYPTION_KEY: Buffer.alloc(32, 1).toString('base64'), AI_CREDENTIAL_ENCRYPTION_KEY: Buffer.alloc(32, 2).toString('base64'), PAYMENT_CREDENTIAL_ENCRYPTION_KEY: Buffer.alloc(32, 3).toString('base64'), INTEGRATION_CREDENTIAL_ENCRYPTION_KEY: Buffer.alloc(32, 4).toString('base64'), REPORT_SIGNING_KEY: 'production-report-signing-key-with-entropy', METRICS_TOKEN: 'production-metrics-token-with-high-entropy', APP_URL: 'https://app.zerochack.example', NEXT_PUBLIC_API_URL: 'https://api.zerochack.example/v1', CORS_ORIGINS: 'https://app.zerochack.example', SMTP_HOST: 'smtp.zerochack.example', SMTP_FROM: 'security@zerochack.example' };
